...
namespace NanjingUniversity.CppMonitor.DAO.imp
{
    class ContentLoggerImpl:ILoggerDao
    {
        ...
        public override void ensureTableExist()
        {
            ...
            conn.Open();
            //建立content_info
            string sql = "create table if not exists content_info (id INTEGER, time char[22],operation char[7],fullpath TEXT,textfrom blob,textto blob,line int,lineoffset int,happentime int8,project Text)";
            SQLiteCommand cmd = new SQLiteCommand(sql, conn);
            cmd.ExecuteNonQuery();
            conn.Close();
        }
